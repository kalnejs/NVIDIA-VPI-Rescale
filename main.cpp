...
std::string gstreamer_pipeline (int id, int mode, int capture_width, int capture_height, int display_width, int display_height, int framerate, int flip_method) {
    return "nvarguscamerasrc sensor-id=(int)" + std::to_string(id) + " sensor-mode=(int)" + std::to_string(mode) + " ! video/x-raw(memory:NVMM), width=(int)" + std::to_string(capture_width) + ", height=(int)" +
